...
<style>
  ...
  .header-box { background:#f8f9fa; padding:12px 16px; border-radius:8px; border:1px solid #78a3faff;}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function compareDateYMD(a,b){ if(a===b) return 0; return a<b?-1:1; }
  ...
  // ===== column defs =====
  const columnDefs = [
    { headerName:'', field:'_actions', width:80, sortable:false, filter:false,
      cellRenderer: (p) => {
        ...
        const delBtn = document.createElement('button'); delBtn.textContent = 'ー';
        wrap.appendChild(addBtn); wrap.appendChild(delBtn);
        ...
        delBtn.addEventListener('click', async () => {
          const row = p.data;
          if (!row.id) { gridApi.applyTransaction({remove:[row]}); return; }
    ...
    { headerName:'日付', field:'expense_date', width:120, editable:false,
      valueGetter:(p)=> toYMDLocal(p.data.expense_date),
      comparator:(a,b)=> compareDateYMD(a,b),
      sort: 'asc',       // ★ 初期ソート
      sortIndex: 0,      // ★ 優先順位1位
    },
    { headerName:'Weekday', field:'weekday', width:100, editable:false,
      valueGetter:(p)=> {
    ...
    // 入力列
    { headerName:'From', field:'station_from', width:140, editable:true },
    { headerName:'To', field:'station_to',   width:140, editable:true },
    { headerName:'Amount', field:'cost', width:100, editable:true,
      valueFormatter:(p)=> p.value!=null ? Number(p.value).toLocaleString() : '',
      valueParser:(p)=> Number(p.newValue ?? 0),
    },
    { headerName:'Type', field:'trip_type', width:100, editable:true,
      cellEditor:'agSelectCellEditor', cellEditorParams:{ values:['round_trip','one_way'] }
    },
    { headerName:'！', field:'category', width:100, editable:true,
      cellEditor:'agSelectCellEditor', cellEditorParams:{ values:['regular','irregular'] }
    },
